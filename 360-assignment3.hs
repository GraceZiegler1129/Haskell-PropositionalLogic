...
getAtoms (ATOM v)          vList = vList ++ [v]
getAtoms (NOT phi)         vList = nub (getAtoms phi vList)
...
evalF valu (ATOM c)            = if lookup c valu == Just True then True else False
evalF valu (AND phi1 phi2)     = evalF valu phi1 && evalF valu phi2
evalF valu (OR phi1 phi2)      = evalF valu phi1 || evalF valu phi2
evalF valu (IMPLIES phi1 phi2) = evalF valu phi1 <= evalF valu phi2
...
prove ctx phi = ((elem_of phi (decompose [] ctx)) || (elem_of BOT (decompose [] ctx)) || (prove_right (decompose [] ctx) phi))

-- Function I wrote to cycle through a list and see if element is in a list
elem_of :: Formula -> Context -> Bool
elem_of _ [] = False
elem_of x (ar : ars) = if x == ar then True else elem_of x ars
...
decompose ctx1 (middle : ctx2) =
  case middle of
    ...
    IMPLIES phi psi -> 
      if (prove (ctx1 ++ ctx2) phi) then decompose ctx1 (psi : ctx2)
